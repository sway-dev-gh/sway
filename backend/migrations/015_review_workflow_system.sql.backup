-- SwayFiles Review & Approval Workflow System Migration
...
-- File sections for granular review (new core feature)
CREATE TABLE IF NOT EXISTS file_sections (
    ...
    section_type VARCHAR(50) DEFAULT 'content' CHECK (section_type IN ('content', 'design', 'code', 'documentation', 'metadata')),
    ...
    assigned_reviewers INTEGER[] DEFAULT '{}',
...
-- Section reviews and approvals (extends existing review system)
CREATE TABLE IF NOT EXISTS section_reviews (
    id SERIAL PRIMARY KEY,
    section_id INTEGER REFERENCES file_sections(id) ON DELETE CASCADE,
...
-- Enhanced section comments system (extends existing comments)
CREATE TABLE IF NOT EXISTS section_comments (
    id SERIAL PRIMARY KEY,
    section_id INTEGER REFERENCES file_sections(id) ON DELETE CASCADE,
    parent_comment_id INTEGER REFERENCES section_comments(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS file_versions (
    ...
    created_from_version_id INTEGER REFERENCES file_versions(id),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX IF NOT EXISTS idx_file_sections_project_id ON file_sections(project_id);
CREATE INDEX IF NOT EXISTS idx_file_sections_section_order ON file_sections(file_id, section_order);
CREATE INDEX IF NOT EXISTS idx_file_sections_status ON file_sections(section_status);
...
CREATE TRIGGER update_file_sections_updated_at BEFORE UPDATE ON file_sections FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
